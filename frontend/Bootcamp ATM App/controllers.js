...
atmApp.controller('bpinController', ['$scope', '$http', '$window', '$timeout', function ($scope, $http, $window, $timeout) {
    ...
    $http.get('http://localhost:8080/getusers/1').
        then(function (response) {
            ...
            console.log(dbPin); //check if able to get the value
...
atmApp.controller('wpinController', ['$scope', '$http', '$window', function ($scope, $http, $window) {
    // > w-scc
}]);
atmApp.controller('opinController', ['$scope', '$http', '$window', function ($scope, $http, $window) {
    // > receipt
}]);

// PIN CHANGE
atmApp.controller('pinchangeController', ['$scope', '$window', '$timeout', function ($scope, $window, $timeout) {

    $scope.invalidMsg = false;
    $scope.response = function () {
        var code = $scope.d1 + $scope.d2 + $scope.d3 + $scope.d4 + $scope.d5 + $scope.d6;
        if ((code) == 123456) { //COMPARE TO DB
